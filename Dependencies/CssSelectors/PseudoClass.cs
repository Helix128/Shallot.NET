    ...
    public abstract class PseudoClass
    {
        ...
        public static PseudoClass GetPseudoClass(string pseudoClass)
        {
            if (!Classes.ContainsKey(pseudoClass))
                throw new NotSupportedException($"Pseudo class {pseudoClass} not supported.");
        ...
        private static Dictionary<string, PseudoClass> LoadPseudoClasses()
        {
            ...
            var types = Assembly.Load(new AssemblyName(System.Reflection.Assembly.GetEntryAssembly().GetName().Name)).GetTypes().Where(i => !i.GetTypeInfo().IsAbstract && i.GetTypeInfo().IsSubclassOf(typeof(PseudoClass)));
